...
export function AgentLog() {
    ...
    <section className="panel"><h2>{t("Активная задача")}</h2><form className="inline-form" onSubmit={e => { e.preventDefault(); checkJob(); }}><label>{t("Идентификатор задачи")}<input aria-label={t("Идентификатор задачи")} required value={jobInput} onChange={e => setJobInput(e.target.value)} placeholder="job_id"/></label><button className="job-action-button" disabled={!requestedJobId || checking} aria-busy={checking}>{checking && <span className="spinner" aria-hidden="true"/>}{t(checking ? "Проверяем…" : "Проверить задачу")}</button></form>{jobId && <JobStatus key={jobId} id={jobId} requestVersion={jobRequestVersion} onLoadingChange={handleJobLoading}/>}</section>
  </>;
}
