...
export function SourcesPage() {
    ...
    <div className="page-heading"><div><h1>{t("Источники данных")}</h1></div><a className="button primary" href="#csv-import">{t("+ Импорт CSV")}</a></div>
    ...
    <details className="panel content-details source-status"><summary>{t("Состояние источников")}</summary><div className="source-grid">{sources.data?.map(source => <section className="panel source-card" key={source.id}><div className="inline-heading"><span className="source-icon">▤</span><Badge status={source.status}/></div><h2>{source.name}</h2><p>{source.type}</p><dl className="metadata"><dt>{t("Обновлено")}</dt><dd>{source.updated_at ? `${dateLabel(source.updated_at, timezone)} ${timezone}` : t("Нет данных")}</dd><dt>{t("Покрытие")}</dt><dd>{source.coverage == null ? t("Нет данных") : `${numberLabel(source.coverage * 100, 1)}%`}</dd></dl>{source.error && <p className="warning-text">{source.error}</p>}{source.status === "planned" && <p>{t("Подключение запланировано. Данные не поступают.")}</p>}</section>)}</div></details>
    ...
    <section id="csv-import" className="panel"><div className="panel-heading"><div><h2>{t("Импортировать историю")}</h2><p>{t("Проверьте колонки и параметры файла перед загрузкой.")}</p></div></div>
    <ResourceNotice {...assets}/><form onSubmit={submit} onChange={() => setConfirmed(false)}>
      ...
      <p className="muted">{t("Мощность — исходная нормализованная шкала, ветер — м/с, температура — °C. Февральские данные используются только для оценки.")}</p>
      <label className="checkbox" onChange={e => e.stopPropagation()}><input type="checkbox" checked={confirmed} onChange={e => setConfirmed(e.target.checked)}/>{t(" Подтверждаю колонки, часовой пояс, метки времени и единицы")}</label>
      ...
      {transport === "fixture" && <p className="muted">{t("Демо: файл не отправляется, отчёт содержит пример данных.")}</p>}
