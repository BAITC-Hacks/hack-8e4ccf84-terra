...
export const pointSchema = z.object({ target_time: timestamp, lead_hour: z.number().int().positive(), prediction: z.number().finite().nullable(), actual: z.number().finite().nullable(), status: z.string(), interval: z.tuple([z.number().finite(), z.number().finite()]).refine(([low, high]) => low <= high).nullable().optional() });
