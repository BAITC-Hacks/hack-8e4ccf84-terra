...
function ForecastContent() {
    ...
    return <>
    <div className="page-heading"><div><div className="eyebrow">{t("ПРОГНОЗ / ПОЧАСОВАЯ МОЩНОСТЬ")}</div><h1>{t("Прогноз выработки")}</h1><p>{t("Каждый час — с источником, версией и объяснением.")}</p></div><button className="primary" disabled={!selected || busy} onClick={exportForecast}>{t("↓ Экспорт CSV")}</button></div>
    ...
    {selected && <>
      ...
      <div className="panel-foot">{t("Экспорт содержит весь сохранённый горизонт (")}{selected.horizon_hours}{t(" ч), время UTC, единицы и идентификаторы версии.")}</div></section>
      <div className="two-columns"><section className="panel briefing"><div className="eyebrow">{t("✳ БРИФИНГ")}</div><h2>{t("Что стоит за прогнозом")}</h2><p>{selected.briefing}</p>{mode === "replay" && <div className="notice warning">{t("Симуляция. Виртуальный выпуск: ")}{dateLabel(selected.issued_at, timezone)} {timezone}.</div>}</section><section className="panel"><h2>{t("Происхождение результата")}</h2><dl className="metadata"><dt>{t("Версия")}</dt><dd>{selected.id}</dd><dt>{t("Модель")}</dt><dd>{selected.model_version}</dd><dt>{t("Погодный прогон")}</dt><dd>{selected.weather_run_id}</dd><dt>{t("Снимок входов")}</dt><dd>{selected.input_snapshot_id}</dd><dt>{t("Предыдущая версия")}</dt><dd>{previous?.id ?? t("Нет данных")}</dd><dt>{t("Единица")}</dt><dd>{t("Нормализованная мощность, исходная шкала")}</dd></dl></section></div>
    </>}
    <section className="panel"><div className="panel-heading"><div><h2>{mode === "backtest" ? t("Запуск бэктеста") : mode === "replay" ? t("Расчёт в симуляции") : t("Новый прогноз")}</h2><p>{transport === "fixture" ? t("Демонстрация принятия задачи. Реальные расчёты не запускаются.") : t("Сервер проверит модель, временную шкалу и доступность входов.")}</p></div></div>
