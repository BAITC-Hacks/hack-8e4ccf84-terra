...
const navigation: [string, string, IconName][] = [["/history", "История прогнозов", "forecast"], ["/overview", "Обзор", "overview"], ["/forecast", "Прогноз", "forecast"], ["/sources", "Источники", "sources"], ["/agent-log", "Журнал агента", "activity"]];
    ...
    <div className={`app-shell ${menuOpen ? "menu-open" : ""}`}>
      ...
      <div className="workspace" inert={menuOpen}>
        ...
        <div className="environment-controls">
          <label>{t("Данные")}<select aria-label={t("Данные")} value={transport} onChange={e => setTransport(e.target.value as Transport)}><option value="fixture">{t("Демонстрационные")}</option><option value="api">{t("Подключённые источники")}</option></select></label>
          ...
          {transport === "fixture" && <details className="demo-options"><summary>{t("Демосценарии")}</summary><label>{t("Сценарий UI")}<select aria-label={t("Сценарий UI")} value={scenario} onChange={e => setScenario(e.target.value as Scenario)}>{[["ready", "Готово"], ["partial", "Частичные данные"], ["stale", "Устарело"], ["loading", "Загрузка"], ["empty", "Пусто"], ["error", "Ошибка"]].map(([value, label]) => <option key={value} value={value}>{t(label)}</option>)}</select></label></details>}
          {path !== "/history" && <span className={`mode-pill ${mode}`}>{mode === "replay" ? t("СИМУЛЯЦИЯ · REPLAY") : mode.toUpperCase()}</span>}
        </div>
        <div className={`environment-banner ${transport === "api" ? "api" : ""}`} role="status"><Icon name={transport === "api" ? "shield" : "sources"} size={16} /><strong>{t(transport === "fixture" ? "Демонстрационные данные" : "Данные источников")}</strong><span>{t(transport === "fixture" ? "Учебные данные, не реальные измерения." : "Данные подключённых систем.")}</span></div>
