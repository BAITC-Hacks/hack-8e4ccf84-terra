...
function Setup({kind, client, assets, transport}: {kind: IndustrialKind; client: DashboardClient; assets: Asset[]; transport: Transport}) {
  ...
  const canEnable = access === "healthy" && !!resource && !!currentAsset && fields.length === definition.mappings.length && new Set(fields).size === fields.length;
  ...
  return <section className="panel industrial-connector" aria-labelledby={`${kind}-title`}>
    ...
    {access === "healthy" && selected && <>
      <div className="form-grid">
        <label>{t(definition.resourceLabel)}<select aria-label={t(definition.resourceLabel)} value={resource} onChange={event => { setResource(event.target.value); setMapping({}); setEnabled(undefined); }}>{resources.map(item => <option key={item.name} value={item.name}>{item.name}</option>)}</select></label>
        <label>{t("Турбина")}<select aria-label={`${t(definition.title)} · ${t("Турбина")}`} value={currentAsset} onChange={event => { setAssetId(event.target.value); setEnabled(undefined); }}>{assets.map(asset => <option key={asset.id} value={asset.id}>{asset.name}</option>)}</select></label>
      </div>
      <h3>{t("Сопоставление полей с турбиной")}</h3>
      <div className="mapping-grid">{definition.mappings.map(item => <label key={item.key}>{t(item.label)}<select aria-label={`${t(definition.title)} · ${t(item.label)}`} value={mapping[item.key] ?? ""} onChange={event => { setMapping({...mapping, [item.key]: event.target.value}); setEnabled(undefined); }}><option value="">{t("Выберите поле или тег")}</option>{selected.fields.map(field => <option key={field} value={field}>{field}</option>)}</select></label>)}</div>
    ...
    {enabled && <div className="notice success" role="status"><strong>{t(kind === "oracle" ? "История загружается" : "Обновления поступают")}</strong><span>{t("Турбина:")} {assets.find(asset => asset.id === currentAsset)?.name ?? currentAsset} · {t("источник:")} {resource}</span></div>}
  </section>;
}

export function IndustrialConnectors({client, assets, transport}: {client: DashboardClient; assets: Asset[]; transport: Transport}) {
  return <div className="industrial-grid"><Setup kind="oracle" client={client} assets={assets} transport={transport} /><Setup kind="wincc" client={client} assets={assets} transport={transport} /></div>;
}
