...
export function Login({ returnTo }: { returnTo: string }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/auth/session", { method: "POST", credentials: "same-origin", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ password }), signal: AbortSignal.timeout(15_000) });
      if (!response.ok) {
        setPassword("");
  ...
  return <div className="login-page">
    ...
    <main className="login-main">
      ...
      <section className="login-card"><div className="login-lock"><Icon name="lock" size={24} /></div><div className="eyebrow">{t("ЗАЩИЩЁННЫЙ ДОСТУП")}</div><h2>{t("Вход в рабочее пространство")}</h2><p>{t("Войдите с учётной записью администратора вашей станции.")}</p><form onSubmit={submit}><label>{t("Учётная запись")}<input value={t("Администратор станции")} readOnly autoComplete="username" /></label><label htmlFor="password">{t("Пароль администратора")}</label><div className="password-field"><input id="password" type={visible ? "text" : "password"} autoComplete="current-password" required value={password} onChange={e => setPassword(e.target.value)} disabled={busy} placeholder={t("Введите пароль")} aria-invalid={!!error} aria-describedby={error ? "login-error" : undefined} /><button type="button" aria-label={t(visible ? "Скрыть пароль" : "Показать пароль")} onClick={() => setVisible(!visible)}><Icon name="eye" size={18} /></button></div>{error && <div id="login-error" className="notice danger" role="alert">{t(error)}</div>}<button className="primary login-submit" disabled={busy || !password}>{busy ? <><span className="spinner" />{t("Проверяем доступ…")}</> : <>{t("Войти")}<Icon name="arrow" size={18} /></>}</button></form><div className="login-security"><Icon name="shield" size={16} /><span>{t("Доступ только для авторизованных пользователей")}</span></div><p className="login-help">{t("Нет доступа? Обратитесь к администратору вашей организации.")}</p></section>
